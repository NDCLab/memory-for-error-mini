...
compCluster = [22, 24, 53, 55]; 
 
%define timeRange to average over for component 
compTime = [0 50]; %the target arrow appears 150 ms after face appearance and the marker is sent that time. This is why I have 0 to 50 ms to compute n170 erp from150-200 ms (relative to face appearance)
...
%range of interest
stim_incon_error_avgTime = mean(newData(:,1,:,compRange),4);
stim_incon_corr_avgTime = mean(newData(:,2,:,compRange),4);

%average cluster of interest
stim_incon_error_avgTimeClust = mean(stim_incon_error_avgTime(:,:,compCluster),3);
stim_incon_corr_avgTimeClust = mean(stim_incon_corr_avgTime(:,:,compCluster),3);

%compute difference scores
stim_incon_error_avgTimeClust_diff = stim_incon_error_avgTimeClust - stim_incon_corr_avgTimeClust;

%write sub numbers to ouput
output(:,1) = subjects_id;

%write component means to output
output(:,2) = stim_incon_error_avgTimeClust;
output(:,3) = stim_incon_corr_avgTimeClust;
output(:,4) = stim_incon_error_avgTimeClust_diff;

output = table(output(:,1), output(:,2), output(:,3), output(:,4));
output.Properties.VariableNames = {'id','error_N170','correct_N170','deltaN170'};

%write component means to disk
writetable(output, [output_location filesep 'mini_mfe_erpDat_N170_compMeans.csv'])
