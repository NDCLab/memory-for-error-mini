...
main_dir = '/home/data/NDClab/datasets/social-flanker-eeg-dataset';

% Setting up other things

%Location of MADE and ADJUSTED-ADJUST scripts
addpath(genpath([main_dir filesep 'code' filesep 'mini_mfe_eeg' filesep 'MADE-EEG-preprocessing-pipeline']));% enter the path of the folder in this line

%Location of "EEG
addpath(genpath([main_dir filesep 'code' filesep 'mini_mfe_eeg' filesep 'eeglab13_4_4b']));% enter the path of the EEGLAB folder in this line

%remove path to octave functions inside matlab to prevent errors when
rmpath([main_dir filesep 'code' filesep 'mini_mfe_eeg' filesep 'eeglab13_4_4b' filesep 'functions' filesep 'octavefunc' filesep 'signal'])

% 1. Enter the path of the folder that has the data to be analyzed
data_location = [main_dir filesep 'derivatives' filesep 'mini_mfe_eeg' filesep 'preprocessed' filesep 'processed_data'];
...
for i = 1:length(datafile_names)
    ...
    % loop through 7 conditions (combinations of event types)
    for c = 1:7
        ...
        EEG1 = pop_selectevent( EEG, 'latency','-1<=1','eventType',eventType,'congruency',congruency, 'Accuracy',Accuracy,'deleteevents','on','deleteepochs','on','invertepochs','off');
        ...
        mini_mfe_erpDat(pIdx,c,:,:)= meanEpochs;
...
save('mini_mfe_erpDat.mat','mini_mfe_erpDat', 'subject')
...
load('mini_mfe_erpDat.mat')


%make a copy/rename the erp matrix 
allData = mini_mfe_erpDat;
...
%pull out 7 conditions of interest for all subs
resp_incon_err = chan(:,1,:,:);
resp_incon_corr = chan(:,2,:,:);
resp_con_corr = chan(:,3,:,:);

stim_incon_err = chan(:,4,:,:);
stim_incon_corr = chan(:,5,:,:);
stim_con_err = chan(:,6,:,:);
stim_con_corr = chan(:,7,:,:);
