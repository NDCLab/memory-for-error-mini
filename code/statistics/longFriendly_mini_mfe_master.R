...
# Last Update: 2023-01-24 (YYYY-MM-DD)
...
main_df <- setNames(data.frame(matrix(ncol = 84, nrow = 0)), c("participant_id", "congAcc", "incongAcc", "congCorr_meanRT", "incongCorr_meanRT", "congCorr_logMeanRT", "incongCorr_logMeanRT",
                                                               ...
                                                               "percent_pre_incong_correctFaces_reported_friendly", "percent_pre_incong_errorFaces_reported_friendly",
                                                               "percent_post_incong_errorFaces_reported_friendly", "percent_post_incong_correctFaces_reported_friendly",
                                                               "percent_incong_corrFaces_reported_friendly", "percent_incong_errorFaces_reported_friendly",
                                                               ...
                                                               "pre_correct_hitRate", "post_correct_hitRate" , "correct_hitRate" , "error_hitRate", "num_incong_errors", "num_incong_corrects",
                                                               ...
                                                               "num_post_incong_correctFaces_reported_new", "num_error_faces_in_surp", "pre_error_trials_in_surp", "post_error_trials_in_surp", "error_trials_in_surp", "pre_correct_trials_in_surp", "correct_trials_in_surp", "post_correct_trials_in_surp"))
  ...
  if (participant_included == 1){
    ...
    if (nrow(incong_errorDat) >= incong_error_cutoff){ # checks to see if the participant has at least 6 errors or not.
      ...
      if (num_error_faces_in_surp >= incong_error_cutoff){
        ...
        if (!is.na(num_incong_errorFaces_reported_old)){
        ...
        if (length(percent_incong_errorFaces_reported_old) ==0){
        ...
        if (!is.na(num_incong_corrFaces_reported_old)){
        ...
        if (length(percent_incong_corrFaces_reported_old) == 0){
        ...
        if (nrow(incong_errorDat) != 0){
          ...
          # Loop over the faces from error trials.
          for (hhhh in 1:nrow(incong_errorDat)){
            temp_face <- incong_errorDat$straightFace[hhhh]
          ...
          num_incong_errorFaces_reported_friendly <- NA
        }
        if (!is.na(num_incong_errorFaces_reported_friendly)){
          num_incong_errorFaces_reported_unfriendly <- nrow(incong_errorDat) - num_incong_errorFaces_reported_friendly # The number of correct faces that they report as new
        ...
        if (length(percent_incong_errorFaces_reported_friendly) == 0){
          percent_incong_errorFaces_reported_friendly <- NA
          percent_incong_errorFaces_reported_unfriendly <- NA
        } else {
        ...
        if (nrow(incong_corrDat) != 0){
          ...
          for (tttt in 1:nrow(incong_corrDat)){
            temp_face <- incong_corrDat$straightFace[tttt]
            temp_for_friendly <- filter(participant_friendlyDat, surpriseFaces == temp_face) # find the error face in the friendlyDat
            if (nrow(temp_for_friendly) == 1){
              identified_friendly <- ifelse (temp_for_friendly$FriendlyKey == temp_for_friendly$friendly_key_resp.keys, 1, 0) #returns 1 when participant identifies the face as friendly!
              if (identified_friendly == 1){
          ...
          num_incong_corrFaces_reported_friendly <- NA
        }
        if (!is.na(num_incong_corrFaces_reported_friendly)){
          num_incong_corrFaces_reported_unfriendly <- nrow(incong_corrDat) - num_incong_corrFaces_reported_friendly # The number of correct faces that they report as new
        ...
        if (length(percent_incong_corrFaces_reported_friendly) == 0){
          percent_incong_corrFaces_reported_friendly <- NA
          percent_incong_corrFaces_reported_unfriendly <- NA
        } else {
        ...
        # The goal is to create a data frame of post-error faces. post error faces must be correct.
        ...
        if (!is.na(num_post_incong_errorFaces_reported_old)){
        ...
        if (!is.na(num_post_incong_correctFaces_reported_old)){
        ...
        if (!is.na(num_pre_incong_errorFaces_reported_old)){
        ...
        if (!is.na(num_pre_incong_correctFaces_reported_old)){
        ...
        if (is.null(num_post_correct_faces)){
          num_post_correct_faces <- NA
        }
        num_pre_correct_faces <- nrow(pre_correct_faces)
        if (is.null(num_pre_correct_faces)){
          num_pre_correct_faces <- NA
        }
        num_pre_error_faces <- nrow(pre_error_faces)
        if (is.null(num_pre_error_faces)){
          num_pre_error_faces <- NA
        }
        num_post_error_faces <- nrow(post_error_faces)
        if (is.null(num_post_error_faces)){
          num_post_error_faces <- NA
        }
        ...
        if (!is.null(post_error_faces)){
          ...
          for (tttt in 1:nrow(post_error_faces)){
            temp_face <- post_error_faces$straightFace[tttt]
            temp_for_friendly <- filter(participant_friendlyDat, surpriseFaces == temp_face) # find the error face in the friendlyDat
            if (nrow(temp_for_friendly) == 1){
              identified_friendly <- ifelse (temp_for_friendly$FriendlyKey == temp_for_friendly$friendly_key_resp.keys, 1, 0) #returns 1 when participant identifies the face as friendly!
              if (identified_friendly == 1){
          ...
          num_post_incong_errorFaces_reported_friendly <- NA
        }
        if (!is.na(num_post_incong_errorFaces_reported_friendly)){
          num_post_incong_errorFaces_reported_unfriendly <- nrow(post_error_faces) - num_post_incong_errorFaces_reported_friendly # The number of correct faces that they report as new
        ...
        if (length(percent_post_incong_errorFaces_reported_friendly) == 0){
          percent_post_incong_errorFaces_reported_friendly <- NA
          percent_post_incong_errorFaces_reported_unfriendly <- NA
        } else {
        ...
        if (!is.null(post_correct_faces)){
          ...
          for (tttt in 1:nrow(post_correct_faces)){
            temp_face <- post_correct_faces$straightFace[tttt]
            temp_for_friendly <- filter(participant_friendlyDat, surpriseFaces == temp_face) # find the error face in the friendlyDat
            if (nrow(temp_for_friendly) == 1){
              identified_friendly <- ifelse (temp_for_friendly$FriendlyKey == temp_for_friendly$friendly_key_resp.keys, 1, 0) #returns 1 when participant identifies the face as friendly!
              if (identified_friendly == 1){
          ...
          num_post_incong_correctFaces_reported_friendly <- NA
        }
        if (!is.na(num_post_incong_correctFaces_reported_friendly)){
          num_post_incong_correctFaces_reported_unfriendly <- nrow(post_correct_faces) - num_post_incong_correctFaces_reported_friendly # The number of correct faces that they report as new
        ...
        if (is.na(percent_post_incong_correctFaces_reported_friendly)){
          percent_post_incong_correctFaces_reported_friendly <- NA
          percent_post_incong_correctFaces_reported_unfriendly <- NA
        } else {
        ...
        if (!is.null(pre_error_faces)){
          ...
          for (tttt in 1:nrow(pre_error_faces)){
            temp_face <- pre_error_faces$straightFace[tttt]
            temp_for_friendly <- filter(participant_friendlyDat, surpriseFaces == temp_face) # find the error face in the friendlyDat
            if (nrow(temp_for_friendly) == 1){
              identified_friendly <- ifelse (temp_for_friendly$FriendlyKey == temp_for_friendly$friendly_key_resp.keys, 1, 0) #returns 1 when participant identifies the face as friendly!
              if (identified_friendly == 1){
          ...
          num_pre_incong_errorFaces_reported_friendly <- NA
        }
        if (!is.na(num_pre_incong_errorFaces_reported_friendly)){
        ...
        if (length(percent_pre_incong_errorFaces_reported_friendly) == 0){
          percent_pre_incong_errorFaces_reported_friendly <- NA
          percent_pre_incong_errorFaces_reported_unfriendly <- NA
        } else {
        ...
        if (!is.null(pre_correct_faces)){
          ...
          for (tttt in 1:nrow(pre_correct_faces)){
            temp_face <- pre_correct_faces$straightFace[tttt]
            temp_for_friendly <- filter(participant_friendlyDat, surpriseFaces == temp_face) # find the error face in the friendlyDat
            if (nrow(temp_for_friendly) == 1){
              identified_friendly <- ifelse (temp_for_friendly$FriendlyKey == temp_for_friendly$friendly_key_resp.keys, 1, 0) #returns 1 when participant identifies the face as friendly!
              if (identified_friendly == 1){
          ...
          num_pre_incong_correctFaces_reported_friendly <- NA
        }
        if (!is.na(num_pre_incong_correctFaces_reported_friendly)){
        ...
        if (length(percent_pre_incong_correctFaces_reported_friendly) == 0){
          percent_pre_incong_correctFaces_reported_friendly <- NA
          percent_pre_incong_correctFaces_reported_unfriendly <- NA
        } else {
        ...
        pre_error_trials_in_surp <- num_pre_incong_errorFaces_reported_old + num_pre_incong_errorFaces_reported_new
        post_error_trials_in_surp <- num_post_incong_errorFaces_reported_old + num_post_incong_errorFaces_reported_new
        error_trials_in_surp <- num_incong_errorFaces_reported_old + num_incong_errorFaces_reported_new
        pre_correct_trials_in_surp <- num_pre_incong_correctFaces_reported_old + num_pre_incong_correctFaces_reported_new
        correct_trials_in_surp <- num_incong_corrFaces_reported_old + num_incong_corrFaces_reported_new
        post_correct_trials_in_surp <- num_post_incong_correctFaces_reported_old + num_post_incong_correctFaces_reported_new
        ...
        main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc, congCorr_meanRT, incongCorr_meanRT, congCorr_logMeanRT, congErr_logMeanRT,
                                        ...
                                        num_incong_errors, num_incong_corrects, num_post_correct_faces, num_pre_correct_faces, num_pre_error_faces, num_post_error_faces,
...
for (zesht4 in 1:nrow(main_df)){
  if (!is.na(as.numeric(main_df$memoryBias_score[zesht4])) && !is.na(as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$percent_incong_corrFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$percent_incong_errorFaces_reported_friendly[zesht4])) && !is.na(as.numeric(main_df$error_hitRate[zesht4])) && !is.na(as.numeric(main_df$correct_hitRate[zesht4])) && !is.na(as.numeric(main_df$hitRate_post_error_minus_pre_error[zesht4])) && !is.na(as.numeric(main_df$hitRate_error_minus_pre_error[zesht4])) && !is.na(as.numeric(main_df$post_error_hitRate[zesht4])) && !is.na(as.numeric(main_df$post_correct_hitRate[zesht4])) && !is.na(as.numeric(main_df$scaared_b_scrdSoc_s1_r1_e1[zesht4])) && !is.na(as.numeric(main_df$scaared_b_scrdTotal_s1_r1_e1[zesht4])) && !is.na(as.numeric(main_df$scaared_b_scrdGA_s1_r1_e1[zesht4])) && !is.na(as.numeric(main_df$hitRate_error_minus_correct[zesht4])) && !is.na(as.numeric(main_df$hitRate_post_error_minus_correct[zesht4])) && !is.na(as.numeric(main_df$bfne_b_scrdTotal_s1_r1_e1[zesht4]))){
    ...
    if (as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_pre_incong_correctFaces_reported_friendly[zesht4] <- NA
    }

    if (as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_pre_incong_errorFaces_reported_friendly[zesht4] <- NA
    }

    if (as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_post_incong_errorFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_post_incong_errorFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_post_incong_errorFaces_reported_friendly[zesht4] <- NA
    }

    if (as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_post_incong_correctFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_post_incong_correctFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_post_incong_correctFaces_reported_friendly[zesht4] <- NA
    }

    if (as.numeric(main_df$percent_incong_corrFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_incong_corrFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_incong_corrFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_incong_corrFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_incong_corrFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_incong_corrFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_incong_corrFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_incong_corrFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_incong_corrFaces_reported_friendly[zesht4] <- NA
    }

    if (as.numeric(main_df$percent_incong_errorFaces_reported_friendly[zesht4]) >= (mean(as.numeric(main_df$percent_incong_errorFaces_reported_friendly), na.rm = TRUE) - (3*sd(as.numeric(main_df$percent_incong_errorFaces_reported_friendly), na.rm = TRUE))) && as.numeric(main_df$percent_incong_errorFaces_reported_friendly[zesht4]) <= (mean(as.numeric(main_df$percent_incong_errorFaces_reported_friendly), na.rm = TRUE) + (3*sd(as.numeric(main_df$percent_incong_errorFaces_reported_friendly), na.rm = TRUE)))){
      main_df$percent_incong_errorFaces_reported_friendly[zesht4] <- as.numeric(main_df$percent_incong_errorFaces_reported_friendly[zesht4])
    } else {
      main_df$percent_incong_errorFaces_reported_friendly[zesht4] <- NA
    }
...
for (j in 1:nrow(main_df)){
  if (is.na(main_df$num_post_correct_faces[j]) || main_df$num_post_correct_faces[j] < incong_error_cutoff ){
    main_df$post_correct_hitRate[j] <- NA
    main_df$post_d_prime_correct[j] <- NA
  }
  if (is.na(main_df$num_pre_correct_faces[j]) || main_df$num_pre_correct_faces[j] < incong_error_cutoff ){
    main_df$pre_correct_hitRate[j] <- NA
    main_df$pre_d_prime_correct[j] <- NA
  }
  if (is.na(main_df$num_pre_error_faces[j]) || main_df$num_pre_error_faces[j] < incong_error_cutoff ){
    main_df$pre_error_hitRate[j] <- NA
    main_df$pre_d_prime_error[j] <- NA
  }
  if (is.na(main_df$num_post_error_faces[j]) || main_df$num_post_error_faces[j] < incong_error_cutoff ){
...
long_df <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), c("id", "accuracy", "position", "hitRate", "dPrime", "friendliness_percentage", "condition_trial_num", "congAcc", "incongAcc", "congCorr_logMeanRT", "incongCorr_logMeanRT", "flankEff_meanACC", "flankEff_logMeanRT", "incongErr_logMeanRT", "congErr_logMeanRT")) # will store data of all participants
participant_long_df <- setNames(data.frame(matrix(ncol = 15, nrow = 6)), c("id", "accuracy", "position", "hitRate", "dPrime", "friendliness_percentage", "condition_trial_num", "congAcc", "incongAcc", "congCorr_logMeanRT", "incongCorr_logMeanRT", "flankEff_meanACC", "flankEff_logMeanRT", "incongErr_logMeanRT", "congErr_logMeanRT"))
for(rrr in 1:nrow(main_df)){
  #pre_error
  zzz <- 1 # this is a row counter
  participant_long_df$id[zzz] <- main_df$participant_id[rrr]
  participant_long_df$accuracy[zzz] <- 0
  participant_long_df$position[zzz] <- -1
  participant_long_df$hitRate[zzz] <- main_df$pre_error_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$pre_d_prime_error[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$pre_error_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_pre_incong_errorFaces_reported_friendly[rrr]
  ...
  participant_long_df$hitRate[zzz] <- main_df$error_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$d_prime_error[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$error_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_incong_errorFaces_reported_friendly[rrr]
  participant_long_df$congAcc[zzz] <- main_df$congAcc[rrr]
  participant_long_df$incongAcc[zzz] <- main_df$incongAcc[rrr]
  participant_long_df$congCorr_logMeanRT[zzz] <- main_df$congCorr_logMeanRT[rrr]
  participant_long_df$incongCorr_logMeanRT[zzz] <- main_df$incongCorr_logMeanRT[rrr]
  ...
  participant_long_df$accuracy[zzz] <- 0
  participant_long_df$position[zzz] <- 1
  participant_long_df$hitRate[zzz] <- main_df$post_error_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$post_d_prime_error[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$post_error_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_post_incong_errorFaces_reported_friendly[rrr]
  participant_long_df$congAcc[zzz] <- main_df$congAcc[rrr]
  participant_long_df$incongAcc[zzz] <- main_df$incongAcc[rrr]
  participant_long_df$congCorr_logMeanRT[zzz] <- main_df$congCorr_logMeanRT[rrr]
  participant_long_df$incongCorr_logMeanRT[zzz] <- main_df$incongCorr_logMeanRT[rrr]
  ...
  zzz <- zzz + 1
  participant_long_df$id[zzz] <- main_df$participant_id[rrr]
  participant_long_df$accuracy[zzz] <- 1
  participant_long_df$position[zzz] <- -1
  participant_long_df$hitRate[zzz] <- main_df$pre_correct_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$pre_d_prime_correct[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$pre_correct_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_pre_incong_correctFaces_reported_friendly[rrr]
  participant_long_df$congAcc[zzz] <- main_df$congAcc[rrr]
  participant_long_df$incongAcc[zzz] <- main_df$incongAcc[rrr]
  participant_long_df$congCorr_logMeanRT[zzz] <- main_df$congCorr_logMeanRT[rrr]
  participant_long_df$incongCorr_logMeanRT[zzz] <- main_df$incongCorr_logMeanRT[rrr]
  ...
  participant_long_df$accuracy[zzz] <- 1
  participant_long_df$position[zzz] <- 0
  participant_long_df$hitRate[zzz] <- main_df$correct_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$d_prime_correct[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$correct_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_incong_corrFaces_reported_friendly[rrr]
  participant_long_df$congAcc[zzz] <- main_df$congAcc[rrr]
  participant_long_df$incongAcc[zzz] <- main_df$incongAcc[rrr]
  participant_long_df$congCorr_logMeanRT[zzz] <- main_df$congCorr_logMeanRT[rrr]
  participant_long_df$incongCorr_logMeanRT[zzz] <- main_df$incongCorr_logMeanRT[rrr]
  ...
  participant_long_df$accuracy[zzz] <- 1
  participant_long_df$position[zzz] <- 1
  participant_long_df$hitRate[zzz] <- main_df$post_correct_hitRate[rrr]
  participant_long_df$dPrime[zzz] <- main_df$post_d_prime_correct[rrr]
  participant_long_df$condition_trial_num[zzz] <- main_df$post_correct_trials_in_surp[rrr]
  participant_long_df$friendliness_percentage[zzz] <- main_df$percent_post_incong_correctFaces_reported_friendly[rrr]
  participant_long_df$congAcc[zzz] <- main_df$congAcc[rrr]
  participant_long_df$incongAcc[zzz] <- main_df$incongAcc[rrr]
  participant_long_df$congCorr_logMeanRT[zzz] <- main_df$congCorr_logMeanRT[rrr]
  participant_long_df$incongCorr_logMeanRT[zzz] <- main_df$incongCorr_logMeanRT[rrr]
